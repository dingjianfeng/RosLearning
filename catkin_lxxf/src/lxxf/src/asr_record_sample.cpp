...
const char * GRM_FILE            = "test.bnf"; //构建离线识别语法网络所用的语法文件
...
void asrProcess()
{
	const char *login_config    = "appid = 58ec36a0"; //登录参数
	UserData asr_data; 
	int ret                     = 0 ;
	char c;
...
int main(int argc, char* argv[])
{
	...
	ros::Subscriber sub =nd.subscribe("/voice/xf_xfm_topic",1,asrCallBack);  //第二个参数是队列的长度
	ros::Publisher pub =nd.advertise<std_msgs::String>("/voice/xf_nlp_topic",3);    //3是发布队列的长度，如果消息发布过快，那么这个设置就是缓存的消息数，在当前设置下，它将缓冲最多3条消息。
